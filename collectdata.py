...
import os
import cv2
cap=cv2.VideoCapture(0)#this is used to get the camera running
directory='Image/'#this is used to create dir for the saved data futher down
while True:#to loop the cam
    _,frame=cap.read()#to read and display the cam
    # this count function is basically giving the no of images being save dinto the file on press
    count = {
             'a': len(os.listdir(directory+"/Hello")),
             'b': len(os.listdir(directory+"/I love you")),
             'c': len(os.listdir(directory+"/No")),
             'd': len(os.listdir(directory+"/Thanks")),
             'e': len(os.listdir(directory+"/Yes")),
    ...
    # the code from here is mostly to have a count of how many images have been saved
    ...
    if interrupt & 0xFF == ord('b'):
        cv2.imwrite(directory+'I love you/'+str(count['b'])+'.png',frame)
    if interrupt & 0xFF == ord('c'):
        cv2.imwrite(directory+'No/'+str(count['c'])+'.png',frame)
    if interrupt & 0xFF == ord('d'):
        cv2.imwrite(directory+'Thanks/'+str(count['d'])+'.png',frame)
    if interrupt & 0xFF == ord('e'):
        cv2.imwrite(directory+'Yes/'+str(count['e'])+'.png',frame)
